...
function Start-Scrcpy {
    ...
    do {
        ...
        Write-DebugLog "Command: scrcpy $($finalArgs -join ' ')"
        Write-Host "`nCommand: scrcpy $($finalArgs -join ' ')" -ForegroundColor Gray
        ...
        try {
            ...
            if ($DebugPreference -ne "SilentlyContinue" -or $RealTimeCapture) {
                ...
                if ($combinedOutput.Count -gt 0) {
                    Write-DebugLog "Scrcpy output:`n$($combinedOutput -join "`n")"
                }
            }
            else {
